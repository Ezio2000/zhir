...
impl Receiver {
    fn settle(&mut self, result: std::result::Result<Result<()>, oneshot::error::RecvError>) {
        ...
        self.error = match result {
            ...
            Err(_) => Some(Error::Protocol("exchange driver lost its terminal result".into())),
        };
    }
}
...
impl Events {
    async fn send(&self, body: SessionEventBody) -> Result<()> {
        ...
        self.tx
            .send(SessionEvent { sequence: *sequence, body })
            .await
            .map_err(|_| Error::Cancelled)?;
        *sequence = sequence.checked_add(1).ok_or_else(|| Error::Protocol("event sequence overflow".into()))?;
        Ok(())
    }
}
struct Deltas {
...
impl DeltaSink for Deltas {
    fn emit(&self, delta: ModelDelta) -> BoxFuture<'_, Result<()>> {
        Box::pin(async move {
            ...
            self.events.send(SessionEventBody::Delta {
...
pub(crate) fn open(
    ...
    if open.binding.is_some() {
        return Err(Error::Invalid("binding does not belong to an exchange adapter".into()));
    }
    if open.recovery.is_some() {
        return Err(Error::Protocol("exchange session has no persistent session resume".into()));
    }
    ...
    let task = SessionTask {
        events: Events { tx: event_tx, sequence: Arc::new(Mutex::new(0)) },
        exchange,
...
pub(crate) fn validate_capabilities(capabilities: &CapabilitySet) -> Result<()> {
    if [
        ...
        Capability::Resume,
    ].iter().any(|capability| capabilities.supports(*capability)) {
        return Err(Error::Invalid("exchange session advertises unsupported session capabilities".into()));
    }
    Ok(())
}
        ...
        loop {
            ...
            if sending.is_none() {
                let next = if let Some(body) = controls.pop_front() {
                    ...
                    Some((body, kind))
                } else if let Some(finish) = &mut finishing {
                    Some(finish.next())
                } else {
                    None
                };
    ...
    fn command(&mut self, command: SessionCommand, busy: bool) -> Result<Vec<SessionEventBody>> {
        ...
        match command.body {
            SessionCommandBody::Generate {
                generation_id, context_revision, input_position, profile_revision,
            } => {
                if busy || generation_id.is_empty() || context_revision != self.context_revision
                    || input_position != self.input_position || profile_revision != self.profile_revision
                {
                ...
                self.request.messages.extend(conversation(self.history.entries()));
                ...
                self.active = Some((generation_id.clone(), input_position,
                    (self.exchange)(self.request.clone(), context)));
                Ok(vec![acknowledge, SessionEventBody::ResponseStarted { generation_id, input_position }])
            }
            SessionCommandBody::Append { entry, context_revision, input_position, source } => {
                if source == AppendSource::Submitted && self.sealed
                    && matches!(entry.message, zhir_core::message::Message::User { .. })
                {
                    return Err(Error::Invalid("user input is sealed".into()));
                }
                if context_revision != self.context_revision + 1 || input_position < self.input_position {
            ...
            SessionCommandBody::ReplaceContext { entries, context_revision } => {
                if busy || context_revision != self.context_revision + 1 {
                    return Err(Error::Protocol("context replacement crosses a generation or revision".into()));
                }
            ...
            SessionCommandBody::Close => {
                ...
                Ok(vec![acknowledge, SessionEventBody::Closed {
                    reason: "host_request".into(),
                    provider_data: serde_json::Value::Null,
                }])
            }
            _ => Err(Error::Invalid("unsupported exchange session command".into())),
        }
    }
}
...
impl FinishEvents {
    fn new(generation_id: String, input_position: u64, response: GenerationOutput) -> Result<Self> {
        ...
        let effective = response.provider_data.get("effective").cloned()
        ...
        let finished = SessionEventBody::ResponseFinished {
            generation_id: generation_id.clone(), input_position, response_status,
            usage: response.usage, model_id: response.model_id, response_id: response.response_id,
            finish_reason: response.finish_reason, provider_data: response.provider_data, effective,
        };
        Ok(Self { generation_id, output: response.output.into_iter().enumerate(), finished: Some(finished) })
    }
    fn next(&mut self) -> (SessionEventBody, Emission) {
        if let Some((index, output)) = self.output.next() {
            (SessionEventBody::Output {
                ...
                caller_id: "model".into(), output,
            }, Emission::Output)
        } else {
            (self.finished.take().expect("one response completion"), Emission::Finish)
        }
    }
}
fn response_status(response: &GenerationOutput) -> ResponseStatus {
    ...
    if response.output.iter().any(|item| matches!(item, zhir_core::message::Output::RuntimeToolCall { .. })) {
