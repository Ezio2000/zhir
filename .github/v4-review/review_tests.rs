...
use zhir_models::{FunctionModel, decorators::{FallbackCandidate, FallbackModel}};
...
fn command(id: &str, body: SessionCommandBody) -> SessionCommand {
    SessionCommand { id: id.into(), body }
}
fn generate() -> SessionCommand {
    command("generate", SessionCommandBody::Generate {
        generation_id: "g".into(), context_revision: 0, input_position: 0, profile_revision: 0,
    })
}
async fn receive(session: &mut ModelSession) -> SessionEvent {
    tokio::time::timeout(Duration::from_secs(3), session.output.receive())
        .await.expect("session stalled").expect("session error").expect("unexpected EOF")
}
async fn finish(session: &mut ModelSession) {
    session.input.send(command("close", SessionCommandBody::Close)).await.unwrap();
...
async fn encoded_checkpoint_matches_published_schema_and_rejects_old_versions() {
    let runtime = Runtime::builder(Arc::new(FunctionModel::new(
        zhir_testing::model_capabilities(), |_, _| async { Ok(GenerationOutput::text("done")) },
    ))).build().unwrap();
    let checkpoint = runtime.start(RunRequest::new([Message::user("A")])).unwrap()
        .result().await.unwrap().into_checkpoint();
    ...
    assert!(invalid.validate().is_err(), "stale input coverage must not validate as Completed");
}
        ...
        let model = SessionModel::new(caps, move |open, mut peer| {
            ...
            async move {
                peer.event(SessionEventBody::ResponseStarted {
                    generation_id: "auto-a".into(), input_position: open.input_position,
                }).await?;
                signal.notify_one();
                ...
                loop {
                    ...
                    match &c.body {
                        SessionCommandBody::Append { input_position, .. } => latest = *input_position,
                        SessionCommandBody::SealUserInput => {
                            peer.acknowledge(&c, None).await?;
                            break;
                        }
                        _ => return Err(Error::Protocol("unexpected explicit generation or early close".into())),
                    }
                    peer.acknowledge(&c, None).await?;
                }
                assert!(latest > open.input_position);
                peer.finished("auto-a".into(), ResponseStatus::Completed).await?;
                ...
                peer.event(SessionEventBody::ResponseStarted {
                    generation_id: "auto-b".into(), input_position: latest,
                }).await?;
                peer.event(SessionEventBody::Output {
                    generation_id: Some("auto-b".into()), item_id: "answer-b".into(),
                    caller_id: "model".into(), output: Output::text("answer B"),
                }).await?;
                peer.event(SessionEventBody::ResponseFinished {
                    generation_id: "auto-b".into(), input_position: latest,
                    response_status: ResponseStatus::Completed, usage: Default::default(),
                    model_id: None, response_id: None, finish_reason: None,
                    provider_data: Value::Null, effective: Default::default(),
                }).await?;
                peer.close().await
            }
        });
        let runtime = Runtime::builder(Arc::new(model)).build().unwrap();
        let mut invocation = runtime.start(RunRequest::new([Message::user("A")]).mode(mode)).unwrap();
        invocation.start();
        tokio::time::timeout(Duration::from_secs(3), async {
            started.notified().await;
            invocation.control().input(Message::user("B"), "test").await.unwrap();
            invocation.control().seal_user_input().await.unwrap();
            let checkpoint = invocation.result().await.unwrap().into_checkpoint();
            assert!(matches!(checkpoint.state, State::Completed { .. }), "{:?}", checkpoint.state);
            assert_eq!(checkpoint.metrics.generation_requests, 0);
            assert_eq!(checkpoint.metrics.observed_responses, Some(2));
            assert_eq!(checkpoint.active.session.generated_input_position, checkpoint.active.session.input_position);
        }).await.expect("automatic response stalled");
    }
}
...
async fn bounded_delta_and_command_ack_keep_each_other_runnable() {
    ...
    let model = FunctionModel::new(zhir_testing::model_capabilities(), move |_, context| {
        ...
        async move {
            ...
            sink.emit(ModelDelta::Text { output_index: 0, text: "first".into() }).await?;
            ...
            signal.notify_one();
            sink.emit(ModelDelta::Text { output_index: 0, text: "second".into() }).await?;
    ...
    assert!(matches!(receive(&mut session).await.body, SessionEventBody::Ready { .. }));
    session.input.send(generate()).await.unwrap();
    while !matches!(receive(&mut session).await.body, SessionEventBody::ResponseStarted { .. }) {}
    tokio::time::timeout(Duration::from_secs(3), blocked.notified()).await.unwrap();
    session.input.send(command("append", SessionCommandBody::Append {
        entry: HistoryEntry { id: "late".into(), origin: None, message: Message::user("B") },
    ...
    tokio::time::timeout(Duration::from_secs(3), session.input.send(command("seal", SessionCommandBody::SealUserInput)))
        .await.unwrap().unwrap();
    ...
    loop {
        ...
        match event.body {
            ...
            SessionEventBody::Acknowledged { command_id, .. } if command_id == "append" => append_ack = true,
...
async fn cancellation_settles_even_when_output_is_not_consumed() {
    ...
    let model = FunctionModel::new(zhir_testing::model_capabilities(), move |_, context| {
        ...
        async move {
            ...
            sink.emit(ModelDelta::Text { output_index: 0, text: "first".into() }).await?;
            signal.notify_one();
            sink.emit(ModelDelta::Text { output_index: 0, text: "blocked".into() }).await?;
    ...
    cancellation.cancel();
    tokio::time::timeout(Duration::from_secs(3), dropped_rx).await.unwrap().unwrap();
    let mut errors = 0;
    loop {
        match tokio::time::timeout(Duration::from_secs(3), session.output.receive()).await.unwrap() {
            Ok(Some(_)) => (),
    ...
    fn open_session(&self, open: SessionOpen) -> BoxFuture<'_, Result<ModelSession>> {
        Box::pin(async move {
            ...
            if self.unavailable.load(Ordering::SeqCst) {
                return Err(Error::Model(Failure { code: "unavailable".into(), message: "fixture".into(), retryable: true }));
            }
            let name = self.name;
...
fn candidate(name: &'static str, unavailable: bool) -> Arc<Candidate> {
    ...
    Arc::new(Candidate {
        name, unavailable: Arc::new(AtomicBool::new(unavailable)),
        opens: Arc::new(AtomicUsize::new(0)), capabilities,
    })
}
#[tokio::test]
async fn local_projection_binding_survives_checkpoint_recovery_and_reordering() {
    ...
    let fallback = FallbackModel::new(vec![
        FallbackCandidate::new("b", b.clone()), FallbackCandidate::new("a", a.clone()),
    ]).unwrap();
    ...
    let runtime = Runtime::builder(Arc::new(model)).store(store.clone()).build().unwrap();
    let mut invocation = runtime.start(RunRequest::new([Message::user("A")]).mode(RunMode::Interactive)).unwrap();
    invocation.start();
    tokio::time::timeout(Duration::from_secs(3), async {
        loop {
        ...
        invocation.control().pause(zhir_kernel::defaults::pause()).await.unwrap();
    }).await.unwrap();
    let checkpoint = invocation.result().await.unwrap().into_checkpoint();
    assert!(matches!(checkpoint.state, State::Suspended { .. }));
    assert!(checkpoint.active.session.recovery.is_none());
    let binding = checkpoint.active.session.binding.clone().expect("durable candidate binding");
    ...
    let recovered_runtime = Runtime::builder(Arc::new(recovered_model)).store(store.clone()).build().unwrap();
    let before_b = b.opens.load(Ordering::SeqCst);
    let mut invocation = recovered_runtime.resume(
        ResumeRequest::from_checkpoint(Arc::new(restored)).message(Message::user("again"))
    ).await.unwrap();
    invocation.start();
    tokio::time::timeout(Duration::from_secs(3), async {
        loop {
    ...
    assert!(matches!(completed.state, State::Completed { .. }), "{:?}", completed.state);
    assert_eq!(completed.active.session.binding, Some(binding.clone()));
    assert_eq!(b.opens.load(Ordering::SeqCst), before_b, "kernel recovery must not reselect a healthy B");
    assert_eq!(a.opens.load(Ordering::SeqCst), 2);
    let reordered = FallbackModel::new(vec![
        FallbackCandidate::new("a", a.clone()), FallbackCandidate::new("b", b.clone()),
    ]).unwrap();
    ...
    assert_eq!(b.opens.load(Ordering::SeqCst), before_b, "recovery must not retry candidates");
    ...
    assert_eq!(b.opens.load(Ordering::SeqCst), before_b, "missing binding must fail before opening another model");
}
