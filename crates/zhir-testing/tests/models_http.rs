...
use zhir_core::{
    ...
    model::{DeltaSink, Model, ModelContext, ModelDelta, ModelRequest, ToolChoice},
    run::RunContext,
};
...
async fn responses_preserves_provider_output_and_continuation_history() {
    ...
    let response = model
        ...
        .unwrap();
    assert!(
        matches!(&response.output[1],Output::ProviderToolCall {call} if matches!(&call.output[0],Content::Resource {input} if input.resource.modality() == "image"))
    );
    worker.await.unwrap();
...
async fn anthropic_stream_assembles_tool_blocks_and_usage() {
    ...
    let (url, worker) = server(body, "200 OK", true).await;
    let model = anthropic::messages::model(ModelConfig::new(
        url,
        std::sync::Arc::new(zhir_models::credentials::StaticCredential::new(
            "Bearer", "test",
        )),
...
async fn extra_adds_nested_options_without_losing_generated_format_or_stream_options() {
    ...
    for (protocol, extra, expected) in [
        ...
        next.profile.extensions.insert(
            protocol_key(protocol).into(),
            serde_json::from_value(extra.clone()).unwrap(),
        );
                    ...
                    OperationOutcome::Success {
                        content: vec![
                            ...
                            Content::resource(zhir_core::resource::ResourceRef {
                                ...
                                name: None,
                                source: zhir_core::resource::ResourceSource::Url {
    ...
    fn decode(
        ...
        Ok(Some(vec![Output::ProviderToolCall {
            call: zhir_core::message::ProviderToolCall {
                ...
                output: vec![Content::resource(zhir_core::resource::ResourceRef {
                    id: "image".into(),
                    media_type: "image/png".into(),
                    name: None,
...
fn protocol_key(protocol: zhir_models::Protocol) -> &'static str {
    match protocol {
        zhir_models::Protocol::Chat => "chat",
