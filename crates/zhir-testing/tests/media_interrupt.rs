...
async fn probe(late_after_new: bool) {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        caps.features
            .extend([Capability::Duplex, Capability::InterruptOutput]);
        ...
        let mut invocation = runtime
            .start(RunRequest::new(vec![Message::user("speak")]).mode(RunMode::Interactive))
            .unwrap();
        let mut output = invocation.media_output().unwrap();
        invocation.start();
        loop {
            if store.commits().iter().any(|c| {
                c.checkpoint()
                    .active
                    .media
                    .values()
                    .any(|cursor| cursor.sequence == 1)
            }) {
                break;
            }
        ...
        assert!(
            store
                .commits()
                .iter()
                .any(|c| c.checkpoint().revision == receipt.revision
        ...
        if late_after_new {
            ...
            let reference =
                store.commits().last().unwrap().checkpoint().active.media["output:voice:1"]
                    .sealed
                    .clone();
            ...
            assert_eq!(sealed.chunks.last().unwrap().sequence, 1);
            let mut sequences: Vec<_> = sealed.chunks.iter().map(|c| c.sequence).collect();
            if let Some(previous) = sealed.previous {
                let mut reader = resources.open(previous).await.unwrap();
                let mut bytes = Vec::new();
                loop {
                    let part = reader.read(4096).await.unwrap();
                    if part.is_empty() {
                ...
                let previous: SealedMedia = serde_json::from_slice(&bytes).unwrap();
                sequences.splice(0..0, previous.chunks.iter().map(|c| c.sequence));
            }
            assert_eq!(
                sequences,
                [0, 1],
                "late epoch 0 frame severed the epoch 1 chain"
            );
        }
