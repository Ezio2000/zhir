...
use zhir_core::{
    message::Message,
    run::{Checkpoint, Fact, HistoryEntry},
};

fn trace(count: usize) -> Vec<Arc<Checkpoint>> {
    let mut current = zhir_testing::checkpoint(vec![Message::user("initial")]);
    ...
    for count in [1000, 2000, 4000] {
        ...
        let mut samples: Vec<_> = (0..3)
            .map(|_| {
                ...
                zhir_testing::verify_trace(std::hint::black_box(&trace)).unwrap();
