...
async fn schema_binding_and_snapshot_are_shared_for_custom_tools() {
    ...
    let result = catalog
        ...
        .await
        .unwrap();
    assert_eq!(result.final_outcome().structured(), Some(&json!(3)));
...
async fn retry_requires_idempotence_and_retries_transient_errors() {
    ...
    .unwrap();
    assert!(tool.start(call(), context()).await.is_ok());
...
async fn circuit_blocks_during_cooldown_and_recovers_after_a_successful_probe() {
    ...
    assert!(blocked.start(call(), context()).await.is_err());
    assert!(
        matches!(blocked.start(call(), context()).await, Err(zhir_core::error::Error::RuntimeTool(e)) if e.code == "circuit_open")
    );
    assert_eq!(attempts.load(Ordering::SeqCst), 1);
    let recovering = CircuitBreakingTool::new(flaky(true).0, 1, Duration::ZERO).unwrap();
    assert!(recovering.start(call(), context()).await.is_err());
    assert!(recovering.start(call(), context()).await.is_ok());
    assert!(recovering.start(call(), context()).await.is_ok());
}
