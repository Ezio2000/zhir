...
impl Audio {
    ...
    pub fn push(&mut self, session_id: &str, hex: &str) -> Result<MediaChunk> {
        if !self.open {
            return Err(protocol("audio outside a sentence"));
        }
        self.chunk(session_id, decode_audio(hex, self.limit)?, false)
    }
    pub fn end(&mut self, session_id: &str) -> Result<MediaChunk> {
        if !self.open {
            return Err(protocol("sentence ended without an open sentence"));
        }
        let chunk = self.chunk(session_id, vec![], true)?;
        self.open = false;
        Ok(chunk)
    }
    ...
    fn chunk(&mut self, session_id: &str, bytes: Vec<u8>, end: bool) -> Result<MediaChunk> {
